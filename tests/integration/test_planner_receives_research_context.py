...
@verifies(SWR.SWR_129, SWR.SWR_143, SWR.SWR_1536)
def test_planner_receives_research_context_from_prior_sibling_artifacts(
    ...
    assert "PRIOR SIBLING SUMMARIES" in planner.task_payload
    assert "requirements-research" in planner.task_payload
    assert "input-composer-map" in planner.task_payload
    assert "requirements-20260522-shared-artifacts.md" in planner.task_payload
    assert "src/rotaris_core/tui/widgets/input_composer.py" in planner.task_payload
