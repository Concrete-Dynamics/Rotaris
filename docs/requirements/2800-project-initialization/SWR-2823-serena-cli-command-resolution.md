...
Hard-coding the Rotaris-owned Serena launcher in the setup runner would do exactly that: a
...
The result is that upgrading Serena stays the single dependency edit SWR-2819 promises, and a
workspace that overrode the server keeps its override for setup too.

## Acceptance criteria

- The resolved invocation for the shipped default executes the installed pinned
  `serena-agent` runtime through the Rotaris-owned launcher.
