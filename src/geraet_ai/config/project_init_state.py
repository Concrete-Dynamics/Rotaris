...
The workspace config file is ``<workspace>/.geraet-ai/agents.yaml`` — the same
...
:func:`geraet_ai.fs.atomic_write` so a crash mid-write can never truncate the
...
import yaml
from geraet_ai.config.bootstrap import write_minimal_agents_yaml
from geraet_ai.config.schema import InitializationState
from geraet_ai.config.startup_models import agents_yaml_path
from geraet_ai.fs import atomic_write
from geraet_ai.reqtocode import SWR, traces
...
def read_initialization_state(workspace_root: Path) -> InitializationState:
    ...
    :class:`~geraet_ai.config.schema.InitializationState`, whose
