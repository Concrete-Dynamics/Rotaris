...
server's disabled list can therefore hide tools from the prompt while leaving them live tools
that a model can still call.
...
4. **Post-connect probes are unaffected.** Serena's `list_memories` binding probe (SWR-2905)
   reads the server's own unfiltered tool list, so making `disabled_tools` real does not
   disarm it.
...
list, and the `list_memories` probe still finds its tool. Unit coverage for the factory: the
prompt's MCP section lists exactly the granted names, so prompt and runtime cannot disagree.
