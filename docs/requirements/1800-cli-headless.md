...
### Agentic Framework - Prompt & Tool Improvement, Headless CLI Mode (2026-04-13)

Original: `docs/requirement-log/done/requirements-20260413-201248.md` — document status: Complete

#### Description

This document captures improvements to an agentic Python framework in three areas. First, system prompt templates for all agents are to be reworked using patterns distilled from the oh-my-openagent (omo) and oh-my-pi frameworks - specifically their phase-driven orchestration model, role-identity patterns, communication rules, and structured delegation schemas. Second, tool descriptions for the delegation tool and the file-edit tool are to be rewritten to be clearer and more workflow-guiding, reducing agent errors at the tool boundary. Third, the Python application must expose its full existing functionality through argparse so it operates without a UI, and the test suite is to be updated to verify parity between both execution paths.

#### Implementation Notes

**Requirements Document:**

tool behavior, but architect-owned architecture documentation boundaries are amended by `requirements-20260509-120000.md`. > **Consolidation note:** This file now serves as the retained detailed contract for the > prompt/tooling work that was later split across the core requirement docs. REQ-009 through > REQ-013 remain the detailed delegation-tool contract referenced by the core orchestration and > tools docs. REQ-014 through REQ-016 remain the prompt-facing HAET contract and are refined by > `requirements-20260430-haet-overhaul.md`. REQ-017 through REQ-025 remain the retained source > of truth for headless CLI parity, behavior-contract tests, and field-wise config overlay > semantics. Architect ownership of architecture documentation and the architect/docs-writer > boundary are further specified by `requirements-20260509-120000.md`.

**Resolution Items:**

**◆1** `REQ-20260413-201248-014` - [CLARIFY] Does the current file-edit tool already produce hash-annotated output on read, or does implementing hash anchors require modifying the tool's read output format as well? If the latter, this requirement spans both the read and edit tool surfaces and should be split.

**Excluded / Out of Scope:**

- Changes to the underlying LLM provider integration or model selection logic are not in scope; only the prompt content and tool description text are addressed.

- Migration of existing conversation/session history is not in scope.

- UI redesign or new UI features are not in scope.

#### Acceptance Criteria

All requirement rows are implemented.

### Rotaris - `--help` / `-h` flag (2026-04-25)

Original: `docs/requirement-log/done/requirements-20260425-224446.md` — document status: Complete (stdlib `argparse` covers all three subcommands)

#### Description

The console interface shall provide a `--help` / `-h` flag that displays usage instructions, available commands, and argument descriptions. This gives users immediate guidance on how to interact with the tool without needing to consult external documentation.
...
**◆1** `REQ-20260425-224446-001` - [DONE] Leveraging stdlib `argparse` for the headless CLI - `--help`/`-h` works natively. **◆2** `REQ-20260425-224446-002` - [DONE] Audited `argparse_app.py`; all three subcommands (run, sessions, version) and their arguments are registered with help text. **◆3** `REQ-20260425-224446-003` - [DONE] Added `help="Available commands"` to `add_subparsers()` for readable subcommand listing; fixed misleading task argument help text.

**Excluded / Out of Scope:**

- Context-sensitive help (e.g., `--help` per subcommand) is not required for this initial implementation; a single top-level help message suffices.

- Internationalization or localization of help text is out of scope.

- Man-page generation or HTML documentation derivation from the help text is out of scope.

#### Acceptance Criteria

All requirement rows are implemented or explicitly tracked according to status `Complete (stdlib `argparse` covers all three subcommands)`.
