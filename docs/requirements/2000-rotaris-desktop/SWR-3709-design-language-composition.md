...
| Integration | Sidebar refresh routes counts into headers; chip splits path from session | `WorkspaceView._refresh_sidebar` / `TitleBar.refresh` → labels | `test_workspace_sidebar.py`, `test_chrome.py` additions |
